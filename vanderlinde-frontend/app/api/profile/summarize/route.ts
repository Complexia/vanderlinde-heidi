...
export async function POST(req: NextRequest) {
  try {
    ...
    const contentToSummarize = meetings.map(m => m.content.map((c: any) => c.text).join('\n')).join('\n\n---\n\n');
...
If an existing summary is provided, update it with the new information from the latest meeting, otherwise create a new summary from all the notes.
...
${existingSummary ? `Here is the existing summary to update:\n\n${JSON.stringify(existingSummary)}` : ''}`;
